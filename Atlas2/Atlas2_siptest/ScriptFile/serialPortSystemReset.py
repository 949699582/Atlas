...
def main():
	...
	baud = 115200
	delimiter = '*_*'
	command_list = ['SYSTEM_RESET']
	command_json_dic = {}
	for command in command_list:
		timeout = 3
		
		if '1,2,3,4,5,6,7,8,9,1' in command :
			timeout = 10
		command_result = sendCommand(port,baud,delimiter,command,timeout =timeout)
		command_json_dic[command] = command_result
		if 'END_RESET' in command :
			time.sleep(2)
