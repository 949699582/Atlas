...
#        self.debug = True
    ...
    def timeStamp(self):
        timeStep = time.time() - self.initTime
        return "Time " + str(timeStep) + 's: '

    def sendCommand(self,rcode,cmd,payload,addDum=False): #payload is an array []
        if self.debug: print("cmd:",cmd, "payload: ", payload)
        if self.uart.is_open:
            ...
            if self.debug: 
                print(self.timeStamp() + "Send:", [hex(no) for no in list(self.tx)])
        ...
        if self.uart.is_open:
            ...
            while ((time.time()-TimeBegin) < timeout):
                ...
                if(len(self.rx)>=rxdata_Length): 
                    break
            if self.debug: 
                print(self.timeStamp() + 'Receive:',[hex(no) for no in list(self.rx)])
            ...
            if opcode != (cmd & 0x0FFF):
                print("B2P: Response is incorrect! expected %x got %x\n" %(command_check, opcode))
                return False
