...
class grassNinjaHost():
    ...
    def setTimeouts(self, addDum = False):
        '''
        ...
        reg59_dev = self.locustB2P.readLocustReg(REG_CONFIRM_COMMS_TIMEOUT, target='device', n=1, addDum=addDum)[0]
        if reg59_dev >> 5 != 0x7:
            value_to_send = (reg59_dev & 0x1F | 0xE0)
            self.locustB2P.writeLocustReg(REG_CONFIRM_COMMS_TIMEOUT,value_to_send, target='device', addDum=addDum)

        localRemoteTimeoutRegs =[0x36,0x55]

        for reg in localRemoteTimeoutRegs:
            hostCFG= self.locustB2P.readLocustReg(reg, target='host', n=2) # 4:0, 10ms to 320ms, mid_remote_timeout
            devCFG = self.locustB2P.readLocustReg(reg, target='device', n=2, addDum=addDum) # 4:0, 10ms to 320ms, mid_remote_timeout
            hostCFG5= hostCFG[0]
            devCFG5 = devCFG[0]
            hostCFG6= hostCFG[1]
            devCFG6 = devCFG[1]
            ...
            self.locustB2P.writeLocustReg(reg, [hostcfg5,hostcfg6], target='host') # 4:0, 10ms to 320ms, mid_remote_timeout
            self.locustB2P.writeLocustReg(reg, [devcfg5,devcfg6], target='device', addDum=addDum) # 4:0, 10ms to 320ms, mid_remote_timeout
            self.locustB2P.writeLocustReg(REG_CONFIRM_COMMS, 0x01, target='device', addDum=addDum)
            self.locustB2P.writeLocustReg(REG_CONFIRM_COMMS, 0x0, target='device', addDum=addDum)
...
#            if capExist: 
            if 1:  #Dev1 uses 0.1uF cap that is too small for cap detection, it will report a short and failed to continue
                ...
                self.enableCharging()  # added to wake up a DUT from UVP, otherwise the DUT won't be activated
                if PPOn: 
                    ppenable = 'enabled'
                else:
                    self.enableCharging(on=False) 
                    ppenable = 'disabled'
                time.sleep(delay)
                self.locustB2P.sendReset("DeviceLocust", reset_mode=0x01, addDum = True) # reset has to be done before ping to setup linkstate; Otherwise b2p path to device doesn't work
                self.locustB2P.sendPing(destination="DeviceLocust")
                time.sleep(0.1)
                socStatus = self.b2pPingSoC(timeout=5) # Added this in P3 --  it's found SoC firmware boot time can interfere with the GN script; Add this line to wait for firmware bootup
                self.setTimeouts(not socStatus)
    ...
    def enableCharging(self,on=True):
        ...
        if on:
            ...
            self.locustB2P.pullup(False,"HostLocust")
            self.locustB2P.getState()
        else:
            self.locustB2P.pullup(True,"HostLocust")
    ...
    def checkDevicePowerStatus(self,timeout=1):
        #check the GN link status
        ...
        if self.locustB2P.sendPing(destination="DeviceLocust") and self.b2pPingSoC(timeout=timeout) and self.locustI2C.waitUSBActive(timeout=timeout): 
            ...
            print("GN host: link is broken, can't get device power status'")
            return False
                
    def checkBuckSwitchFrequency(self, addDum=False):
        '''
        check DCM counter frequency on bucks
        '''
        self.locustB2P.writeLagunaReg(0x24b2, 1, addDum=addDum) # enable PP2V7
        self.locustB2P.writeLagunaReg(0x7c70, 0x7f, addDum=addDum) # enable buck frequency counter
        time.sleep(0.2)
        self.locustB2P.writeLagunaReg(0x7c71, 0x01, addDum=addDum) # take samples

        regVal = self.locustB2P.readLagunaReg(register=0x7c71, n=1, addDum=addDum)[0]
        while regVal != 0:
            time.sleep(0.1)
            regVal = self.locustB2P.readLagunaReg(register=0x7c71, n=1, addDum=addDum)[0]

        winCounter = self.locustB2P.readLagunaReg(register=0x7c72, n=3, addDum=addDum) # window counter value
        winCounterVal = ((winCounter[2] & 0b11) << 16) + (winCounter[1] << 8) + winCounter[0]
        ...
        for i in range(6):
            regAddr = 0x40f6 + 0x100 * i 
            dcmCounter = self.locustB2P.readLagunaReg(register=regAddr, n=4, addDum=addDum) # dcm counter value 
            dcmCounterVal = ((dcmCounter[3] & 0b11) << 24) + (dcmCounter[2] << 16) + (dcmCounter[1] << 8) + dcmCounter[0]
            ccmCounter = self.locustB2P.readLagunaReg((0x40fa + 0x100 * i), 3, addDum)
            ccmCounterVal = ((ccmCounter[2] & 0b11)<<16) + (ccmCounter[1]<<8) + ccmCounter[0]
            dcmCounters.append(dcmCounterVal)
            ccmCounters.append(ccmCounterVal)
            dcmFreqs.append(int(dcmCounterVal*32768/winCounterVal))
            ccmFreqs.append(int(ccmCounterVal*32768/winCounterVal))

        self.locustB2P.writeLagunaReg(0x7c70, 0x40, addDum=addDum) # disable buck frequency counter
        print("GN host: DCM frequecy - buck0 %dHz, buck1 %dHz, buck2 %dHz, buck3 %dHz, buck4 %dHz, buck5 %dHz" %(dcmFreqs[0], dcmFreqs[1], dcmFreqs[2], dcmFreqs[3], dcmFreqs[4], dcmFreqs[5]))
        print("GN host: CCM frequecy - buck0 %dHz, buck1 %dHz, buck2 %dHz, buck3 %dHz, buck4 %dHz, buck5 %dHz" %(ccmFreqs[0], ccmFreqs[1], ccmFreqs[2], ccmFreqs[3], ccmFreqs[4], ccmFreqs[5]))
        return dcmFreqs

    def getBuckFreq(self, addDum=False):
        targetList = {
                0x00:'buck0',
                0x01:'buck1',
                0x02:'buck2',
        ...
        sinkFreqBucks ={}
        Sinks = [0x03, 0x02, 0x01, 0x00] # 0.8mA and 12.8mA
        msg = ''

        noSinkFreq = self.checkBuckSwitchFrequency(addDum)

        for current in Sinks: 
            msg = msg + 'sink ' + str(current) + '--Freq0-5 (Hz)'
            self.locustB2P.writeLagunaReg(0x7d06,(current & 0x03), addDum) # set sink current
            freqBucks=[]
            for buck in range(6): 
                self.locustB2P.writeLagunaReg(0x7d00, (((buck << 2) + 0b1) & 0xFF), addDum) # enable sink current
                self.locustB2P.writeLagunaReg(0x7c70, 0x7f, addDum) # enable dcm counters
                time.sleep(0.05)
                self.locustB2P.writeLagunaReg(0x7c71, 0x1, addDum) # get sample
                winCounter = self.locustB2P.readLagunaReg(0x7c72, 3, addDum)
                winCounterVal = ((winCounter[2] & 0b11) << 16) + (winCounter[1] << 8) + winCounter[0]
                dcmCounter = self.locustB2P.readLagunaReg((0x40f6 + 0x100 * buck), 4, addDum)
                dcmCounterVal = ((dcmCounter[3] & 0b11) << 24) + (dcmCounter[2] << 16) + (dcmCounter[1] << 8) + dcmCounter[0]
                freqBuck = dcmCounterVal*32768/winCounterVal
                freqBucks.append(freqBuck)
                msg = msg + ', %1.2f' %freqBuck
                self.locustB2P.writeLagunaReg(0x7c70, 0x40, addDum=addDum) # disable dcm counters
                self.locustB2P.writeLagunaReg(0x7d00, (((buck << 2) + 0b0) & 0xFF), addDum=addDum) # disable sink current
            sinkFreqBucks[current]=freqBucks
            msg = msg + '\n'

        for buck in range(6):
            freq = []
            freq.append(noSinkFreq[buck])
            for current in Sinks: 
                freq.append(sinkFreqBucks[current][buck])

            xlist=freq
            ylist=[0, 800000, 1600000, 6400000, 12800000] # in nA
#            ylist=[800000, 1600000, 6400000, 12800000] # in nA
            try: 
                gain,current,rsquad =self._linearFit(xlist, ylist)
                msg = msg + 'buck linearfit '+str(buck)+', gain '+str(gain)+' nA/Hz, current '+str(noSinkFreq[buck]*gain/1000)+' uA'+' , R ' + str(rsquad)+'\n'
            except:
                msg = msg + 'buck linearfit '+str(buck)+' failed. The buck DCM counter is likely zero. Please check CCM counter values.\n'
#            msg = msg + 'buck linearfit '+str(buck)+', gain '+str(gain)+' nA/Hz, current '+str(-current/1000)+' uA'+' , R ' + str(rsquad)+'\n'
        print(msg)
        return msg

    def getLDOCurrent(self,n=0, addDum=True):
        '''
        n: integer, LDO (0-3)
        '''
        targetList = {
        ...
        vChannelLDOs = [40,41,42,43] # ldo 0-3
        # screen ldo output voltage first
        voltage = self.lagunaGpadc(40+n)
        if voltage < 0.1:
            msg = "Done, LDO %d is turned off, voltage %f" %(n,voltage)
            print(msg)
            return msg
        else: 
            Sinks = [0x03, 0x02, 0x00] # 0.8mA and 12.8mA
#            Sinks = [0x03, 0x02, 0x01, 0x00] # 0.8mA and 12.8mA
            ADCValues = []
            ch = 4+n
            ILDO = 0
            for i in range(10):
                ILDO += self.lagunaGpadc(ch)
            ILDO = ILDO/10
            ADCValues.append(ILDO)

            for current in Sinks: 
                self.locustB2P.writeLagunaReg(0x7d06,(current & 0x03), addDum) # set sink current
                self.locustB2P.writeLagunaReg(0x7d00, ((((n+6) << 2) + 0b1) & 0xFF), addDum) # enable sink current
                time.sleep(0.05) # sink current settling time
                gpadcReading = self.lagunaGpadc(ch)
                ADCValues.append(gpadcReading)
                self.locustB2P.writeLagunaReg(0x7d00, ((((n+6) << 2) + 0b0) & 0xFF), addDum=addDum) # disable sink current
            
            xlist=ADCValues
#            ylist=[0, 800000, 1600000, 6400000, 12800000] # in nA
            ylist=[0, 800000, 1600000, 12800000] # in nA
            gain,current,rsquad =self._linearFit(xlist, ylist)
            msg = 'Done, LDO ' + str(n)+', gain '+str(gain)+' nA/LSB, current ' + str(gain*ILDO/1000)+' uA'+', R ' +str(rsquad)
            print(msg)
            return gain*ILDO/1000

    def _linearFit(self,x,y):
        '''
        Linear fit y,x
        x, y: list of the same length
        return gain, offset and Rsquad
        '''
        N = len(x)
        num = 0
        denom = 0
        ...
        SST = 0
        for i in range(N):
            y_fit = gain * x[i] + offset
            SSR += (y[i]-y_fit)*(y[i]-y_fit)
            SST += (y[i]-mean_y)**2
    
        RR = 1 - (SSR/SST)
        return gain,offset,RR
    ...
    def KIS2Locust(self,on=True,forceDFU=False,fast=False):
        ...
        if on:
            ...
            time.sleep(0.2)
            self.eUSBDurant(on=True,dfu=forceDFU)
            ...
            self.eUSBDurant(on=False,dfu=False)
...
#            self.parrot.port(on=False)
    ...
    def waitSOCReset(self, timeout=5):
        '''
        This function is used for avoid KIS port drop during SOC RESET;
        The function needs to be called by station test SW,
        as soon as a ’sysmgr reset xxxx’ command is sent to the DUT.
        Once the command finishes, the station can poll the KIS port again — should be the same KIS port.
        '''
    ...
    def forceDFU(self,on=True):
        ...
        readback = self.locustB2P.readLagunaReg(0x0804, 1, True)[0]
        NTries = 1
        while ((readback !=regTarget) and (NTries < 4)):
            print(readback,NTries)
            time.sleep(0.05)
            self.locustB2P.writeLagunaReg(0x0804, regTarget, True)
            print("GN host: linkstate IO status not matching target, retry #%d" %NTries)
            NTries += 1
            readback = self.locustB2P.readLagunaReg(0x0804, 1, True)
        if (NTries < 4):
            return True
        else: 
            return True
    ...
    def getDeviceInfo(self): # DFU mode is required
        ...
        RSP = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=0x02,payload=[0],timeout=0.2, addDum=False)
    ...
    def b2pPingSoC(self, timeout=3): # b2p ping SoC in DFU mode
        ...
        startTime = time.time()
        rsp = False
        dockStatus = self.locustB2P.readLagunaReg(0x0804,1,True)[0]
        if (dockStatus & 0x10) != 0: 
            self.locustB2P.writeLagunaReg(0x0804, (dockStatus & 0xEF), True) # disable kis_dfu_select to enable b2p UART0 path
            print('GN host: !!!!WARNING -- kis_dfu_select was high, b2p to SoC UART0 is blocked. SoC reset is required to enable the path again.')
    ...
    def b2pTransfer(self, opcode, data, timeout=3): # send b2p command to DUT and get response
        print("====== b2p transfer data ======")
        startTime = time.time()
        rsp = False
        while (time.time() - startTime <= timeout):
            if (rsp == False):
                rsp = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=opcode,payload=data,timeout=0.2)
            else:
                print("GN host:",list(rsp))
                print("GN host: SoC b2p responded within %fs" %(time.time()-startTime))
                return rsp
    ...
    def b2pFTVersion(self, timeout=0.3): # b698 b2p function to get ft version
        ...
        if (rsp == False):
            ...
            rsp = list(rsp)
            print("GN host: ft:ok %02X %02X %02X" %(rsp[0], rsp[2], rsp[1])) 
...
# User firmware b2p functions

    def b2pUserSN(self, timeout=0.3): # b698 b2p function to get ft version

        print("====== User FW SN read ======")
        rsp = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=0x0002,payload=[],timeout=0.2) 
        if (rsp == False):
            print("GN host: failed to get correct response",list(rsp)) 
        else: 
            rsp = list(rsp)
    ...
    def b2pBTAddress(self, right = False, timeout=0.3):
        if right:
            command = 0x2164
        else:
            command = 0x1164

        print("====== Get Bluetooth address======")
        rsp = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=command,payload=[],timeout=0.2) 
        if (rsp == False):
            print("GN host: failed to get correct response",list(rsp)) 
        else: 
            ...
            address = list(rsp)
            address.reverse()
            print("GN host: OK: ", end='')
            for byte in address:
                print("%02X" % byte, end='')
            print("")
        return

    def b2pGetSystemInfo2(self, right = False):
        print("====== Get buddy system info2======")
        if right:
            command = 0x206a
        else:
            command = 0x106a
        rsp = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=command,payload=[],timeout=0.2, addDum=False)
        if (rsp == False):
            print("GN host: failed to get correct response",list(rsp)) 
        else: 
            rsp = list(rsp)
    ...
    # Role 1:set to primary, 0:set to secondary
    def b2pRoleSet(self, role = 1, right = False, timeout=0.3): 
        if right:
            command = 0x140c
        else:
            command = 0x240c 
        ...
        print("====== Role Set ======")
        rsp = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=command,payload=[role],timeout=0.3) 
        if (rsp == False):
            print("GN host: failed to get correct response",list(rsp)) 
        else: 
            rsp = list(rsp)
    ...
    def b2pPairingStateSet(self, state = 1, right = False, timeout=0.3): 
        if right:
            command = 0x1402
        else:
            command = 0x2402 

        print("====== Pairing State Set ======")
        rsp = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=command,payload=[state],timeout=0.3) 
        if (rsp == False):
            print("GN host: failed to get correct response",list(rsp)) 
        else: 
            rsp = list(rsp)
    ...
    def b2pBTCoupledAddressSet(self, address = 0, right = False, timeout=0.3): 
        if right:
            command = 0x1464
        else:
            command = 0x2464 

        addr_bytes = address.to_bytes(6, 'little') 

        print("====== Coupled BT Address Set ======")
        rsp = self.locustB2P.b2p_uart.SendRead(rcode=0x0,cmd=command,payload=addr_bytes,timeout=0.3) 
        if (rsp == False):
            print("GN host: failed to get correct response",list(rsp)) 
        else: 
            rsp = list(rsp)
...
#            self.locustB2P.writeLagunaReg(0x24bb,0x03, addDum=True)
    ...
    def ccadcRead(self, addDum=True): # For checking Pliny leakage in HW stress rack

        self.locustB2P.writeLagunaReg(0x24bd,1, addDum=addDum)
        self.locustB2P.writeLagunaReg(0x2807,0, addDum=addDum)
        REG_OPCFG = 0x7c0a # 0xf0 to clear error
        REG_GAIN_I = 0x7c43 # current conversion gain 
        REG_DATA = 0x7c10
        ...
        REG_EOC_EVT = 0x7c40
        self.locustB2P.writeLagunaReg(REG_GAIN_I,0x3, addDum=addDum)
        time_now = time.time()
        for i in range(1):
            self.locustB2P.writeLagunaReg(REG_OPCFG,0xf3, addDum=addDum)
            self.locustB2P.writeLagunaReg(0x7c06,0x0, addDum=addDum) # resolution to 13bits, 125ms
            self.locustB2P.writeLagunaReg(0x7c08,0x0, addDum=addDum) # commute clock to 128Hz
            OPCFG = self.locustB2P.readLagunaReg(REG_OPCFG, addDum=addDum)[0]

            temp = self.locustB2P.readLagunaReg(REG_MEAS_STATUS, addDum=addDum)[0] # read status and get ready for next coversion

            time.sleep(2) # need 2s for ccadc conversion
            self.locustB2P.writeLagunaReg(REG_OPCFG,0x70, addDum=addDum)
    #        print(hex(self.locustB2P.readLagunaReg(0x7c06)[0]))
    #        print(hex(self.locustB2P.readLagunaReg(0x7c08)[0]))
            data = self.locustB2P.readLagunaReg(REG_DATA, 9, addDum=addDum)
            status = data[8]
            if (status & 0x1) and (status & 0x2): 
                voltage = (((data[1] & 0b1111111) << 8) + data[0]) *25 * 0.6/2**16
                current = (((data[3] & 0xff) << 8) + data[2])
                if current >= 0x8000: 
                    current = (0xffff - current) * 10 * 0.3/2**16/2/4
                else:
                    current = current * (-10) * 0.3/2**16/2/4
            else:
                print('CCADC conversion not valid')

        print('time: %f, battery voltage=%fV, current=%fmA, OPCFG=%s' %(time.time()-time_now, voltage, current*1000, hex(OPCFG)))
...
#            newValue = value[0] & (0b11111100) | 0x00 #disable pull down
    ...
    def lagunaGpadc(self, channel=1, agent='a1', addDum=True):
        if agent == 'a1':
            adcCFG = 0x7e00
            adcLSB = 0x7e01
            adcMSB = 0x7e02
        ...
        self.locustB2P.writeLagunaReg(adcCFG,channel, addDum=addDum) #configure manual agent
        time.sleep(0.01) # required, otherwise b2p fail
        dataLSB = self.locustB2P.readLagunaReg(adcLSB, addDum=addDum)[0] # read LSB data register
    
        while (dataLSB >> 7) == 1: # Loop until MSB is 0 (conversion valid)
            time.sleep(0.001)
            dataLSB = self.locustB2P.readlagunaReg(adcLSB, addDum=addDum)[0]
    
        dataMSB = self.locustB2P.readLagunaReg(adcMSB, addDum=addDum)[0]
    ...
    def eUSBDurant(self, on=False, dfu=False):

        if dfu:
            data = 0xd0 # enable kis_dfu_select in DFU mode
        else:
            data = 0xc0 # remove kis_dfu_select in normal application
...
#            self.locustB2P.writeI2C(bus=0x01, slave_addr=LAGUNA_I2C_7BIT_ADDR, data=0x00, wtype="reg16", register=0x1868, destination = "DeviceLocust")
    ...
    def setCommsMode(self, mode="DPM"):
        ...
        # reconfig locust device ISRC to 0.25mA; Fix to remove funbus glitch when toggling powerpath
        readback_reg57_dev = self.locustB2P.readI2CRegs(bus=0x00, slave_addr=0x33, read_len=0x1, register=0x57, rtype="reg8", destination = "DeviceLocust")
        value_to_reg57_dev = (readback_reg57_dev[0] & 0xF3) | 0b0100
        ...
        print("GN host: clear all faults before switching")
        data_w1c = []
        for reg in range(0x40,0x4d,1): data_w1c.append(0xff)
        ...
        while ((NTries > 0) and (PASS == False)): 
            if mode == "DPM":
                ...
                self.locustB2P.sendPing("DeviceLocust") # ping device locust again to reconnect
#                self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=data_w1c, wtype="reg8", register=0x40, destination = "DeviceLocust")
                self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=value_to_reg57_dev, wtype="reg8", register=0x57, destination = "DeviceLocust", get_resp=True) # no response 
                ...
                time.sleep(0.01) # add 10ms delay to avoid dropping data on locust host
                self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=value_to_send_host, wtype="reg8", register=REG_COMMS_Mode, destination = "HostLocust") 
                ...
                NTries -= 1
                if PASS: 
                    self.locustB2P.pullup(True,"DeviceLocust") # enable internal pullup to avoid glitch on funbus
                    print("GN host: set confirm COMMS bit of device succesfully; DPM switch completed")
                else: 
                    print("GN host: failed to set confirm COMMS bit, retry to set confirm COMMS bit of device, #%d" %NTries)
            else: #ASK
                print("GN host: Switching to ASK")
                self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=(value_to_send_host | 0x80), wtype="reg8", register=REG_COMMS_Mode, destination = "HostLocust")  # revert host back to DPM to talk to device
                self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=data_w1c, wtype="reg8", register=0x40, destination = "DeviceLocust")
                time.sleep(0.5) # add some delay to improve stability
                self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=value_to_send_dev, wtype="reg8", register=REG_COMMS_Mode, destination = "DeviceLocust", get_resp=False) 
                time.sleep(0.02) # add some delay to avoid dpm frame error in confirm comms
                self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=value_to_send_host, wtype="reg8", register=REG_COMMS_Mode, destination = "HostLocust")
                time.sleep(0.03) # add some delay to avoid dpm frame error in confirm comms
                PASS = self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=0x1, wtype="reg8", register=REG_CONFIRM_COMMS, destination = "DeviceLocust", timeout=0.2)
...
class locustI2C(): # i2c is not necessary as b2p uart i2c is more efficient 100k vs 1M
    ...
    def reset(self):
        self.masterCheck()
        self.setASKTXParameter(rout=8, timeout=100e-3, amplitude=2) # enable maximum ASK TX amplitude
        return
    ...
    def checkFaultStatus(self):
        ...
        '''
        faultRegs = self.i2c.read(self.addr,0x42,1)
        print("GN host: locust fault status", hex(faultRegs))
        return
    
    def clearFault(self):
        self.writeReg(0x44,0xFF) #clear local and remote timeout reg
        self.writeReg(0x42,0xFF) #clear FIFO and USB errors
        self.writeReg(0x46,0xFF) #clear USB frame errors
        return
...
#        val = self.i2c.read(self.addr,0x1,1)
    ...
    def locustVin(self,on=True):
        '''
        Enable or disable the 4.5V on FUNBUS
        on: bool <True|False>, True- outptut enable, False- output disable
        '''
        if on:
            self.write(0,7,1)
...
if __name__ == '__main__':
    ...
    if not fastMode: 
        gnTest.resetAll()
    if 'off' in funbus4p5: 
        socStatus = gnTest.enableLink(PPOn=False,delay=0, fast=fastMode)
    else: 
        socStatus = gnTest.enableLink(PPOn=True,delay=0, fast=fastMode)

    gnTest.locustI2C.masterCheck()
#    gnTest.b2pPingSoC(timeout=5) # Added this in P3 --  it's found SoC firmware boot time can interfere with the GN script; Add this line to wait for firmware bootup, move this up into enableLink()
    if socStatus: 
        ...
        gnTest.KIS2Locust(on=True,forceDFU=False, fast=fastMode)
        gnTest.locustI2C.waitUSBActive()
        print("GN host: !!!!ERROR -- Because SOC can't be reached (in reset or dead), Grassninja can't enable the USB path")
        print("GN host: !!!!ERROR -- Continue enabling USB for KIS debug. Normal KIS communication port may not function")
