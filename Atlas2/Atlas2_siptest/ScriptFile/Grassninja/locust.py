...
#        print(resp)
    ...
    def sendReset(self, destination = "HostLocust", reset_mode = 0x01, addDum = False):
        '''
        ...
        resp = self.b2p_uart.SendRead(rcode=routing_code[destination], cmd=Cmd["B2P_CMD_RESET"], payload=payload, timeout=0.2, verb=True, get_resp=True, addDum = addDum)
        return resp
    
    def writeI2C(self, bus, slave_addr, data=0x00, wtype="reg8", register=0x00, destination = "HostLocust", timeout=0.4, get_resp=True, addDum=False):
        ...
        if get_resp and (resp == False):
            locustFIFOStatus = self.b2p_uart.SendRead(rcode=routing_code['DeviceLocust'], cmd=0x1C, payload=[0x00,0x33,0x42,2], timeout=timeout, verb=True, addDum=True)  
#            locustFIFOStatus = struct.unpack('B'*2,locustFIFOStatus[2])
            time.sleep(0.01)
            print("GN host - locustb2p: failed to get I2C response, retry one more time, locust FIFO status ", locustFIFOStatus)
            resp = self.b2p_uart.SendRead(rcode=routing_code[destination], cmd=write_type_tosend, payload=complete_data_payload, timeout=timeout, verb=True, get_resp=True, addDum=True)  
    ...
    def readI2CRegs(self, bus, slave_addr, read_len, register=0x00, rtype="reg8", destination = "HostLocust", timeout=0.4, addDum=False):
        read_type = {
            "read": 0x1A,
            "reg8": 0x1C,
            "reg16": 0x1E
        ...
        if resp == False:
            locustFIFOStatus = self.b2p_uart.SendRead(rcode=routing_code[destination], cmd=0x1C, payload=[0x00,0x33,0x42,2], timeout=timeout, verb=True, addDum=True)  
#            print(locustFIFOStatus)
#            locustFIFOStatus = struct.unpack('B'*2,locustFIFOStatus[2])
            time.sleep(0.01)
            print("GN host - locustb2p: failed to get I2C response, retry one more time, locust FIFO status ", locustFIFOStatus)
            resp = self.b2p_uart.SendRead(rcode=routing_code[destination], cmd=read_type_tosend, payload=complete_data_payload, timeout=timeout, verb=True, addDum=True)  
            if resp == False:
                print("GN host - locustb2p[debug]: Error readI2cReg", resp)
                raise RuntimeError("GN host- locustb2p: failed to get I2C response!!!")
    ...
    def pullup(self,on=True, destination="HostLocust"):
        if on:
            print("GN host - locustb2p:","HostLocust local pullup enabled.")
            self.writeI2C(bus=0x00, slave_addr=0x33, data=0x01, wtype="reg8", register=0x4f, destination = destination)            
        else:
            print("GN host - locustb2p:","HostLocust local pullup disabled.")
            self.writeI2C(bus=0x00, slave_addr=0x33, data=0x00, wtype="reg8", register=0x4f, destination = destination)            
        return
    ...
    def getState(self, destination="HostLocust"):
        if 'Device' in destination: 
            rb_data = self.readI2CRegs(bus=0x00, slave_addr=0x33, read_len=0x1, register=0x45, rtype="reg8", destination = destination, addDum=True)
        else:
            rb_data = self.readI2CRegs(bus=0x00, slave_addr=0x33, read_len=0x1, register=0x45, rtype="reg8", destination = destination, addDum=False)
