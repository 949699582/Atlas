...
if __name__ == '__main__': 
    ...
    if 'r' == rw: 
        data = gnTest.locustB2P.readLagunaReg(register=reg)
        print('GN host: laguna device reg %s, data %s' %(hex(reg),hex(data[0])))
    if 'w' == rw:
        data = int(args.data,16) # data in hex, e.g, 0x32
        gnTest.locustB2P.writeLagunaReg(register=reg,data=data)
