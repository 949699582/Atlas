...
if __name__ == '__main__': 
    ...
    parser.add_argument('-r', '--right', help='Querry right bud (defaults to left)', action='store_true', default=False)
    parser.add_argument('-a', '--address', help='Coupled BT address in hex', type=functools.partial(int, base=16), required=True)
    ...
    print("Set address: 0x%06X" % args.address)
    gnTest = grassNinjaHost(b2pUart,i2cPortName)
    gnTest.b2pBTCoupledAddressSet(address = args.address, right = args.right)
