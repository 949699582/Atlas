...
-- user can directly call this table in tech function
Matchbox = {}

-- report failure, but not create error record
-- user can call it to trigger FA/SetPoison/ExitEarly/Retries
-- if trigger FA/SetPoison/ExitEarly, not create error record
-- if trigger Retries, not create error record and skip retries
-- @param msg: string type
function Matchbox.reportFailure(msg)
    if DataReporting.getLocalResult() ~= DataReporting.localResult.fail then
        error('user should create a FAIL record before calling reportFailure')
    end
    -- set shouldCreateErrorRecord as false, tell matchbox not to create error record
    shouldCreateErrorRecord = false
    error(msg)
end

-- a table return key as value; used to handle string input.
local stringConst = {}
setmetatable(stringConst, {__index = function(_, k) return k end})

local ACTION_TECH = 'Matchbox/Tech.lua'
local ACTION_DEBUG = 'Matchbox/Debug.lua'
...
local function addTestToDAG(dag, testName, plugins, test, ...)
    -- run debug action or tech action
    if test.TestName:upper() == 'HALT' and test.Technology == 'CSVDebug' then
        action = ACTION_DEBUG
    else
        action = ACTION_TECH
    end
    ...
    if action == ACTION_DEBUG then
        debugActions[#debugActions + 1] = resolvable
    end
...
function SequenceControl.scheduleTests(dag, mainItems, initialGlobals,
    ...
    -- test: main test item
    for _, test in ipairs(mainItems) do
        ...
        if forkRoot == nil then
            if thread == nil then
                ...
                prev = addTestToDAG(dag, testName, plugins, test, globals, conditions)
            else
                ...
                lastNodeInThreads[thread] = addTestToDAG(dag, testName, plugins, test, globals, conditions)
    ...
    -- limit debug action threadpool to 1
    dag.limit('limit_debug_action_to_1', 1, debugActions)
...
function SequenceControl.executeTechStep(stepInfo, techFunctions, globals, locals, conditions, allowEditingStaticConditions)
    return SequenceControl._executeTechStep(stepInfo, techFunctions, globals, locals, conditions, allowEditingStaticConditions)
end
...
-- define a global value as a flag, initialize it for true
-- if it is true, it will create error record with throwing error
-- if it is false, it will not create error record with throwing error
-- user can call Matchbox.reportFailure() to change flag as false
shouldCreateErrorRecord = true
function SequenceControl._executeTechStep(stepInformation, techFunctions, globals, locals, conditions, allowEditingStaticConditions)
    shouldCreateErrorRecord = true
    local stepInfo = comFunc.clone(stepInformation)
    local tech = stepInfo.Technology
    local testname = stepInfo.TestName
    local actionPlugin, actionFunc = string.match(stepInfo.TestActions, "^([_%a][_%a%d]*)%:([_%a][_%a%d]*)$")
    local limit = stepInfo.limit

    -- saving all limits to be used in FA, because FA is using limits under a different TestName.
    local allLimits = stepInfo.allLimits
    stepInfo.allLimits = nil
    ...
    local isActPass = false

    -- new variable edits; return to be used by debugger
    local returns = {locals = {}, globals = {}, conditions = {}}
    ...
    Log.LogDebug('Inputs: ' .. comFunc.dump(inputStr))
    ...
    if limit then
        -- pass read-only limit table to user
        stepInfo.limit = comFunc.readOnly(limit)
    end
    ...
    if techFunction == nil then
        if actionPlugin == "M" or actionPlugin == "Tech" or actionPlugin == "Common" then
            techFunction = techFunctions[actionPlugin][actionFunc]
            if techFunction == nil then
    ...
    for i = 1, retries do
        ...
        if not isPlugin then
            ...
            ret = {xpcall(techFunctions.M.createPluginRecord, debug.traceback, stepInfo, actionPlugin, actionFunc)}
        end
        isActPass = ret[1]
        if isActPass or not shouldCreateErrorRecord then
            break
        end
    end
    ...
    if retryCount > 1 and shouldCreateErrorRecord then
        local subsubtestname = stepInfo.AdditionalParameters.paraName or stepInfo.AdditionalParameters.subsubtestname
    ...
    -- 2. user call M:startCB and it error out or return false
    ...
    elseif stepInfo.TestActions == 'M:startCB' then
        ...
        conditions.enableSampling = isActPass and ret[2] or false
    elseif stepInfo.TestActions == 'M:forceDisableSampling' then
        if conditions.enableSampling ~= nil then error('Sampling can only be enabled once or disabled once.') end
        -- station cannot use M:startCB needs to write code to check if it is retest MLB, then
    ...
    if not isActPass then
        ...
        if stepInfo.failSubSubTestName ~= nil then
            subsubtestname = 'FA_step_' .. stepIndex .. ' of ' .. stepInfo.failSubSubTestName
        else
            if subsubtestname == nil then
                subsubtestname = 'tech_step_' .. stepIndex .. ' ' .. stepInfo.TestActions
            end
        end
        ...
        return isActPass, errorMsg
        -- end of action fail handling
    else
        ...
        for index, output in ipairs(outputs) do
            ...
            if scope == SCOPE_GLOBAL then
                globals[varName] = value
                returns.globals[varName] = value
            ...
            elseif scope == SCOPE_CONDITION then
                conditionModule.setCondition(varName, value, allowEditingStaticConditions, conditions)
                returns.conditions[varName] = value
            elseif scope == SCOPE_LOCAL then
                locals[varName] = value
                returns.locals[varName] = value
            else
...
function SequenceControl._executeTechFA(itemInfo, globals, locals, conditions)
    ...
    do
        ...
        -- pass limit of FA test to tech lua
...
function SequenceControl._executeTech(itemInfo, globals, conditions, allowEditingStaticConditions)
    ...
    do
        ...
        stepInfo.limit = itemInfo.limits and itemInfo.limits[testname]
        stepInfo.allLimits = itemInfo.limits
...
function SequenceControl._executeTechAndCatchError(func, itemInfo, ...)
    ...
    if not ret[1] then
        ...
        local subtestname = itemInfo.TestName .. (itemInfo.nameSuffix or '')
        ...
        -- ret[2] is updated globals
        -- ret[3] is updated conditions
        return ret[2], ret[3]
    end
end
